...
char* unary_expr(string op, string type, int Number){
    ...
    if(Number==1){ //unary_operator cast_expression
	    ...
	    if (op==string("+")){
		if(is_float(type) || is_Intgr(type)|| type==string("_Complex")|| type==string("_Imaginary") ){;}
		else return NULL;
	     }
	    if (op==string("-")){
		if(is_float(type) || is_Intgr(type)|| type==string("_Complex")|| type==string("_Imaginary") ){;}
...
char* additive_expr(string type1,string type2,char op){
	...
	else if(type1==string("char")&&is_Intgr(type2)) {
		a="char";return a;
	}
        else if(type2==string("char")&&is_Intgr(type1)){
               a="char";return a;
        }
...
char* relational_expr(string type1,string type2,char * op){
     char *a = new char();
     if(is_Intgr(type1)||is_float(type1)||(type1==string("char"))){
         if(is_Intgr(type2)||is_float(type2)||(type2==string("char")))
         {
             a="bool";return a;
         }  
         else if(type2[type2.size()-1]=='*')
         {
             if(is_Intgr(type1)||(type1==string("char"))){ a = "Bool"; 
                        return a; }
             else return NULL;
         }
     }
     if(type1[type1.size()-1]=='*')
       {
             if(is_Intgr(type2)||(type2==string("char"))){ a = "Bool"; 
                  return a; }
             else return NULL;
...
char* conditional_expr(string type1,string type2){
     ...
     if(is_Intgr(type1))          type1 = string("long double");
     if(type2==string("char")) type2 = string("long long");
     if(is_Intgr(type2))          type2 = string("long double");
...
char* bitwise_expr(string type1,string type2){ // ^,&,|
      ...
      if(is_Intgr(type1) || type1==string("bool")){
            if(is_Intgr(type2) || type2==string("bool")){
                a="True"; return a;
            }               
      } 
      return NULL;

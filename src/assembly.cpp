...
void loadArrayElement(pair<string, Entry *> temporary, string registerTmp, int arr_type)
{
    ...
    {
        // cout << "hello\n";
        int k = temporary.second->col_offset.size();
        int prod = 1;
        for (int counter = 1; counter < k; counter++)
            prod *= temporary.second->dim[counter];
        addLine("move $t9, $0");
        for (int counter = 0; counter < k; counter++)
        {
            int index = temporary.second->col_offset[counter];
            if (index < 0 || index == arr_max)
            {
                int value = index < 0 ? index : 0;
                value *= -1;
                addLine("li $t8, " + to_string(value));
            }
            else
            {
                addLine("li $s6, " + to_string(index));
                if (currFunction != "main")
                    addLine("addi $s6, 76");
            ...
            if (counter < k - 1)
            {
                addLine("li $s6, " + to_string(prod));
                addLine("mult $t8, $s6");
                addLine("mflo $t8");
                prod /= temporary.second->dim[counter + 1];
            }
            addLine("add $t9, $t9, $t8");
        }
        addLine("li $t8, 4"); // Assuming int
        addLine("mult $t8, $t9");
        addLine("mflo $t8");
...
void param_code(int i)
{
    ...
    { //cout << emit_list[i].operand_1.first << emit_list[i].operand_1.second->is_array << endl;
        if (emit_list[i].operand_1.second != NULL)
        {
            if (emit_list[i].operand_1.second->is_array == 1 || emit_list[i].operand_1.second->is_array == 2)
            { //array
                cout << "hello\n";
                int k = emit_list[i].operand_1.second->col_offset.size();
                // cout << k << endl;
                int prod = 1;
                for (int counter = 1; counter < k; counter++)
                    prod *= emit_list[i].operand_1.second->dim[counter];
                addLine("move $t9, $0");
                for (int counter = 0; counter < k; counter++)
                {
                    int index = emit_list[i].operand_1.second->col_offset[counter];
                    // cout << index << endl;
                    if (index < 0 || index == arr_max)
                    {
                        int value = index < 0 ? index : 0;
                    ...
                    if (counter < k - 1)
                    {
                        ...
                        addLine("mult $t8, $s6");
                        addLine("mflo $t8");
                        prod /= emit_list[i].operand_1.second->dim[counter + 1];
                    }
                    addLine("add $t9, $t9, $t8");
                }
                addLine("li $t8, 4"); // Assuming int
...
void assign_op_code(int i)
{
    ...
    {
        int k = emit_list[i].ans.second->col_offset.size();
        int prod = 1;
        for (int counter = 1; counter < k; counter++)
            prod *= emit_list[i].ans.second->dim[counter];
        addLine("move $t9, $0");
        for (int counter = 0; counter < k; counter++)
        {
            int index = emit_list[i].ans.second->col_offset[counter];
            // cout << index << endl;
            if (index < 0 || index == arr_max)
            {
                int value = index < 0 ? index : 0;
            ...
            if (counter < k - 1)
            {
                ...
                addLine("mult $t8, $s6");
                addLine("mflo $t8");
                prod /= emit_list[i].ans.second->dim[counter + 1];
            }
            addLine("add $t9, $t9, $t8");
        }
        addLine("li $t8, 4"); // Assuming int
        ...
        else if (emit_list[i].line_no == -1)
        {
            ...
            else if (emit_list[i].op.first == "unary&")
            {
                ...
                {//cout <<"hello\n";
                    ...
                    // if (currFunction != "main")
                    //     addLine("addi $s6, 76");
                    addLine("sub " + reg1 + ", $fp, $s6"); // combine the two components of the address
